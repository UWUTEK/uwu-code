[![MseeP.ai Security Assessment Badge](https://mseep.net/pr/vidwadeseram-uwu-code-badge.png)](https://mseep.ai/app/vidwadeseram-uwu-code)
...
Coding tasks run through **OpenCode Server** (`opencode serve`) with the [**oh-my-opencode**](https://github.com/uwutek/uwu-agent) plugin (built from our uwu-agent fork) for multi-agent orchestration (Odin, Thor, Tyr, Mimir, Saga, Ratatoskr). The plugin provides:
...
| OpenCode + oh-my-opencode | AI coding tool with multi-agent orchestration (Norse god agents from our uwu-agent fork) — runs via `opencode serve` HTTP API |
...
[oh-my-opencode](https://github.com/uwutek/uwu-agent) (npm compat name, source: uwu-agent fork) is built from source and installed into `~/.opencode/` automatically by the installer. It provides:
